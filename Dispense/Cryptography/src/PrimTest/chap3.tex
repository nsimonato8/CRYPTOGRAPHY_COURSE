...
\begin{algorithm}
    ...
    \If{$\exists \alpha, \beta > 1 \in \mathbb{N}: n = \alpha^{\beta}$}{
        \Return{$FALSE$}\;\label{step_1_aks}
    }

    $r \gets \argmin_{r} \operatorname{ord}_{\mathbb{Z}_{r}^{*}}(n) > \lceil \operatorname{log}_{2}^{2}(n) \rceil$\;\label{step_2_aks}

    \For{$b = \{1, \dots, r\}$}{
        \If{$1 < (b,n) < n$}{
            \Return{$FALSE$}\;\label{step_3_aks}
        }
    }

    \If{$n \leq r$}{
        \Return{$TRUE$}\; \label{step_4_aks}
    }

    \For{$b = \{1, \dots, \sqrt{r} \cdot \operatorname{log}_{2}(n)\}$}{
        \If{$(x + b)^{n} \not\equiv x^{n} + b \bmod (x^{r} - 1, n)$}{
            \Return{$FALSE$}\;\label{step_5_aks}
        }
    }
